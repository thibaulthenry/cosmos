        ...
        final Collection<Component> contents = targets
                ...
                .map(target -> {
                    ...
                    try {
                        ...
                        // todo addSuccess();
                        return Component.empty(); // todo return Outputs.ADD_TO_SCORE.asText(amount, target, objective, result);
                    } catch (ArithmeticException ignored) {
                        return Component.empty(); // todo Outputs.OVERFLOWING_OPERATION.asText("Addition", target);
                    }
                })
                .collect(Collectors.toList());

        final TextComponent title = Component.empty(); // todo Outputs.SHOW_SCORE_OPERATIONS.asText(contents.size(), "addition(s)", worldName);
