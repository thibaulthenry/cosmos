...
public class AdvancementListener extends AbstractPerWorldListener implements ScheduledSaveListener {
    ...
    private final AdvancementSerializer serializer;
    ...
    public AdvancementListener(final Injector injector) {
        ...
        this.serializer = injector.getInstance(AdvancementSerializer.class);
    }
    ...
    public void onPostChangeEntityWorldEvent(final ChangeEntityWorldEvent.Post event, @First final ServerPlayer player) {
        ...
        if (!this.groupRegistry.find(Tuple.of(PerWorldFeatures.ADVANCEMENT, originalWorldKey)).map(group -> group.contains(destinationWorldKey)).orElse(false)) {
    ...
    private void save(final ResourceKey worldKey, final ServerPlayer player) {
        this.save(worldKey, player, new AdvancementTreeData(player));
    }

    private void save(final ResourceKey worldKey, final ServerPlayer player, final AdvancementTreeData data) {
        if (this.bypassRegistry.doesBypass(PerWorldFeatures.ADVANCEMENT, player)) {
            return;
        }

        this.groupRegistry.find(Tuple.of(PerWorldFeatures.ADVANCEMENT, worldKey))
                .orElse(Collections.singleton(worldKey))
                .stream()
                .map(key -> this.finderService.findCosmosPath(Directories.ADVANCEMENTS, key, player))
    ...
    private void share(final ResourceKey worldKey, final ServerPlayer player) {
        ...
        this.finderService.findCosmosPath(Directories.ADVANCEMENTS, worldKey, player)
                .flatMap(this.serializer::deserialize)
                .orElse(new AdvancementTreeData())
                .share(player);
    }
